...
let key = 'WYYBZ-F5OCX-PKA4Q-7YX7I-XQPOZ-P7FDJ'; //使用在腾讯位置服务申请的key
...
export default class Detail extends Component {
  ...
  // 打开 路线规划页面
  openRoutePlan(address, latitude, longitude) {
  ...
  componentDidShow() {
    ...
    Taro.cloud
      .callFunction({
        name: 'sceneDetail',  //调用 景点 的云函数，返回景点id
        data: {
          id: sceneId * 1,
        },
      })
  ...
  previewImg(e) {
    ...
    var sceneDetail = _state7.sceneDetail;
    wx.previewImage({ //在新页面中全屏预览图片
      current: sceneDetail.images[index],
      urls: sceneDetail.images,
    })
  }
  ...
  buildHeader() {
    ...
    return (
      ...
      <Swiper className="head-swiper" circular autoplay>
  ...
  openPhoneCall(phone) {
    Taro.makePhoneCall({ phoneNumber: phone }); //调用API，可拨打电话
  }
  ...
  buildContent() {
    ...
    return (
      <View className="content-container" style={`margin-top: -${margin}px`}>
        ...
        <View
          onClick={this.openRoutePlan.bind(this, sceneDetail.address, sceneDetail.latitude, sceneDetail.longitude)}
        ...
        <View className="intro-container">
          ...
          <ScrollView className="scrollview" scrollX scrollWithAnimation>
            {sceneDetail.images.map((image, index) => {
              return (
                <View className="scrollview-item" key={index} data-index="{{index}}" onClick={this.previewImg.bind(this)}>
